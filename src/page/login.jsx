...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (!form.email || !form.password) return setValidErr('Please fill in all fields');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) return setValidErr('Enter a valid email address');
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="bg-card rounded-2xl shadow-xl border border-border/50 p-8">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-muted-foreground text-sm mt-1">Sign in to Amadla Energy</p>
          </div>
          ...
          {/* Demo credentials */}
          <div className="mt-4 p-3 bg-secondary/60 rounded-xl">
            <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Demo accounts</p>
            <div className="flex flex-wrap gap-2">
              {[
                { label: 'Admin', email: 'admin@amadla.energy' },
                { label: 'Operator', email: 'operator@amadla.energy' },
                { label: 'User', email: 'customer@amadla.energy' },
              ].map(({ label, email }) => (
                <button key={email} onClick={() => fillDemo(email)}
                  className="text-xs px-2.5 py-1 rounded-lg bg-accent/10 text-accent hover:bg-accent/20 transition-colors font-medium">
                  {label}
                </button>
              ))}
            </div>
            <p className="text-xs text-muted-foreground mt-1.5">Password: <code className="bg-secondary px-1 rounded">Password123!</code></p>
          </div>
        </div>
      </motion.div>
    </div>
